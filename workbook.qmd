...
Thus, relatedness expands the space where selection favours inbreeding for females, but reduces it for males. 

::: callout-tip
## Note

When $\alpha = 1$, selection will favour males that inbreed in all cases where inbreeding depression \< 1. Hence, the corresponding dark purple line never appears on the figure.
:::
...
**Figure 3**. Inbreeding depression thresholds (before buffering) for varying values of relatedness, male investment in care and the ability of care to buffer inbreeding depression. The dashed line shows the inbreeding threshold when care does not affect inbreeding depression and reproductive investment is female-limited (hereafter referred to as the control condition). 

::: callout-tip
## Note

The rightmost panels show an unusual result. When care is completely effective at buffering inbreeding depression ($\alpha = 1$) and male parental care does not completely replace a future outcrossing opportunity ($\Delta N_\mathrm{m} < 1$), then all levels of care investment predict the same inbreeding depression threshold for a given relatedness coefficient. However, when males simply swap an outcrossing event for an inbred mating ($\Delta N_\mathrm{m} = 1$), inbreeding is selectively neutral. That is, inbreeding and outcrossing are no different, creating a one for one swap. 
:::

## What does the model predict?

1. Male care tends to restrict the invasion of alleles that code for inbreeding, relative to no male care conditions. See the fraction of lines that fall below the dashed, black control line in Figure 3. 

2. Male care reduces the invasion prospects of inbreeding alleles, irrespective of the sex they are expressed in.

3. However, when care has a large mitigating effect on inbreeding depression (large $\alpha$), the conditions for inbreeding alleles to invade become more permissive, even when males invest quite a lot in care. For example, when care reduces inbreeding depression by 75%, and males lose half a future outbred mating, the invasion prospects of an inbreeding strategy are equivalent to the control condition (where substantial inbreeding depression can be tolerated).   

## How we can test this?

This depends on the pliability and specifics of the Burying beetle system. First, males must care to a lesser extent than females and a inbred mating must not completely replace an outbred one i.e. inbreeding must increase lifetime mating success of males. If the degree of care provided by a male can be manipulated without much change in female care and this feeds back into future male mating prospects, then experimental evolution could be very cool. 

Manipulating the effectiveness of care would also be interesting. An experiment with standardised levels of care provided by both parents, but with treatments that vary in care effectiveness, the model predicts a decrease in inbreeding preference. 
